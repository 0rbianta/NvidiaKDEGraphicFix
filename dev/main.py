...
def checkSystemFileExist():
    try:
        ...
        if("sh /usr/lib/pm-utils/power.d/restart_kde" in dump):
        ...
        if(usr==1):
            ...
            restart_file = open("/usr/lib/pm-utils/power.d/restart_kde", "w")
            restart_file.write("killall plasmashell;\nsleep 5\nkstart5 plasmashell")
            restart_file.close()
            modded_dump=dump.replace("        resume_hdparm_spindown", "        resume_hdparm_spindown\n        sh /usr/lib/pm-utils/power.d/restart_kde")
            SYS_File = open("/usr/lib/pm-utils/power.d/95hdparm-apm", "w")
